...
public class PanelInformes extends JPanel {
	...
	public void cargarInformes(List<Alquiler> alquileres, double totalIngresos) {
		// Actualizar tarjetas
		long totalAlq = alquileres.size();
		long activos = alquileres.stream().filter(a -> "activo".equalsIgnoreCase(a.getEstadoAlquiler())).count();
		long pendientes = alquileres.stream()
				.filter(a -> "pendiente_devolucion".equalsIgnoreCase(a.getEstadoAlquiler())).count();
