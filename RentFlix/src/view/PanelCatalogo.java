...
public class PanelCatalogo extends JPanel {
	...
	/**
	 * Carga la tabla con una fila por cada copia disponible. Cada fila lleva la
	 * info de la película + el formato y precio de la copia.
	 */
	public void cargarCopias(List<Pelicula> peliculas, List<Copia> copias) {
		modeloTabla.setRowCount(0);
		for (Copia copia : copias) {
			// Buscar la película que corresponde a esta copia
			Pelicula pelicula = peliculas.stream().filter(p -> p.getId() == copia.getIdPelicula()).findFirst()
					.orElse(null);
			if (pelicula == null)
				continue;
	...
	/**
	 * Devuelve el id de la copia seleccionada. Solo disponible cuando mostrarId =
	 * true. Para vista cliente devuelve -1 (el controlador debe usar
	 * getFilaSeleccionada y reconstruir).
	 */
