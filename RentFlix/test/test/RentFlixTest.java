...
/**
 * Clase de pruebas unitarias para la aplicación RentFlix.
 * <p>
 * Cubre los 9 Casos de Uso del diagrama de casos de uso:
 * </p>
 * <ul>
...
public class RentFlixTest {
	...
	/** DAO de acceso a datos de clientes, usado en los tests de CU01 y CU05. */
	private ClienteDAO clienteDAO;

	/** DAO de acceso a datos de empleados, usado en los tests de CU01. */
	private EmpleadoDAO empleadoDAO;

	/** DAO de acceso a datos de películas, usado en los tests de CU06. */
	private PeliculaDAO peliculaDAO;

	/** DAO de acceso a datos de copias, usado en los tests de CU02, CU03 y CU06. */
	private CopiaDAO copiaDAO;

	/** DAO de acceso a datos de alquileres, usado en los tests de CU02 al CU09. */
	private AlquilerDAO alquilerDAO;

	/** DAO de acceso a datos de pagos, usado en los tests de CU02 y CU07. */
	private PagoDAO pagoDAO;

	/**
	 * Inicializa los DAOs necesarios antes de cada prueba.
	 */
	...
	/** CU01 — Login de cliente con contraseña incorrecta debe devolver null. */
	@Test
	...
	/**
	 * CU01/CU05 — Login de cliente bloqueado. El DAO devuelve el objeto; el
	 * Controlador es quien deniega el acceso. Busca dinámicamente un cliente
	 * bloqueado en la BD.
	 */
	@Test
	public void testLoginClienteBloqueado() {
		...
		Cliente resultado = clienteDAO.login(bloqueado.getNombreUsuario(), bloqueado.getContraseniaCliente());
	...
	/** CU01 — Login de empleado con contraseña incorrecta debe devolver null. */
	@Test
	public void testLoginEmpleadoIncorrecto() {
		Empleado empleado = empleadoDAO.login("laura_emp", "wrongpassword");
		assertNull("Login de empleado con contraseña incorrecta debe devolver null", empleado);
	}
	...
	/**
	 * CU02 — Registrar un alquiler desde el empleado. Usa la primera copia
	 * disponible de la BD para no depender de ids fijos.
	 */
	@Test
	public void testRegistrarAlquilerDesdeEmpleado() {
		...
		Alquiler alquiler = new Alquiler(0, 1, copia.getIdCopia(), 1, idTrans, LocalDate.now().toString(),
				LocalDate.now().plusDays(3).toString(), null, "activo");
		int resultado = alquilerDAO.crear(alquiler);
		assertEquals("Crear alquiler con empleado debe afectar 1 fila", 1, resultado);
	}

	/**
	 * CU02 — Registrar un nuevo cliente con datos únicos debe afectar 1 fila. Usa
	 * un timestamp en el usuario y email para garantizar unicidad.
	 */
	@Test
	public void testRegistrarNuevoCliente() {
		String sufijo = String.valueOf(System.currentTimeMillis());
		Cliente nuevo = new Cliente(0, "Test", "Usuario", "test" + sufijo + "@rentflix.com", "test_" + sufijo, "1234",
				"activo");
		int resultado = clienteDAO.registrar(nuevo);
		assertEquals("Registrar cliente nuevo debe afectar 1 fila", 1, resultado);
	}

	/**
	 * CU02 — Intentar registrar un cliente con usuario ya existente debe lanzar
	 * RuntimeException por violación de unicidad en la BD.
	 */
	@Test(expected = RuntimeException.class)
	public void testRegistrarClienteUsuarioDuplicado() {
		Cliente duplicado = new Cliente(0, "Test", "Duplicado", "otro@email.com", "ana_cli", "1234", "activo");
		clienteDAO.registrar(duplicado);
	}
	 ...
	 * Decisión de diseño: solo acepta "pendiente_devolucion". Busca dinámicamente
	 * un alquiler activo.
	 */
	@Test
	...
	public void testAceptarDevolucionPendienteExitosa() {
		...
		assertTrue("aceptarDevolucion() sobre 'pendiente_devolucion' debe devolver idCopia > 0", idCopia > 0);
	}
	...
	public void testCatalogoSoloCopiaDisponibles() {
		...
		for (Copia copia : copias) {
			assertEquals("Todas las copias del catálogo deben ser 'disponible'", "disponible", copia.getEstado());
		}
	}

	/** CU06 — Búsqueda por título parcial devuelve al menos un resultado. */
	@Test
	...
	/** CU06 — Búsqueda por título inexistente debe devolver lista vacía. */
	@Test
	public void testBuscarPeliculaTituloInexistente() {
		ArrayList<Pelicula> resultados = peliculaDAO.buscarPorTitulo("xxxxxinexistente");
		assertNotNull("La búsqueda no debe devolver null", resultados);
		assertTrue("Búsqueda sin coincidencias debe devolver lista vacía", resultados.isEmpty());
	}
	...
	public void testAlquilerClienteSinEmpleado() {
		...
		Alquiler alquiler = new Alquiler(0, 2, copia.getIdCopia(), null, idTrans, LocalDate.now().toString(),
				LocalDate.now().plusDays(5).toString(), null, "activo");
		int resultado = alquilerDAO.crear(alquiler);
	...
	public void testAlquileresPertenecenAlCliente() {
		...
		for (Alquiler a : alquileres) {
			assertEquals("Todos los alquileres deben pertenecer al cliente 1", 1, a.getIdCliente());
		}
	}
	...
	/**
	 * Integración — Flujo completo: login → pago → alquiler → solicitar devolución
	 * → aceptar devolución → copia disponible. Usa la primera copia disponible de
	 * la BD dinámicamente.
	 */
	@Test
	public void testFlujoCompletoAlquilerDevolucion() {
		...
		// 4. Crear alquiler
		Alquiler alquiler = new Alquiler(0, cliente.getIdCliente(), copia.getIdCopia(), null, idTrans,
				LocalDate.now().toString(), LocalDate.now().plusDays(4).toString(), null, "activo");
